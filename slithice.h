...
namespace slithice {
    ...
    void invert_gray(uchar* in, uchar* out);

    // Invert a RGB32 image.
    void invert(uint* in, uint* out);

    // Builds histogram for a Grayscale8 image.
    void build_hisogram(uchar* in, std::vector<double> &out);

    // Histogram equalization for a Grayscale 8 image.
    void eq_histogram(uchar* in, std::vector<double>& h, uchar* out);

    // Adds 10% of salt and pepper noise to a Grayscale8 image.
    void add_salt_pepper(uchar* in, uchar* out);

    // Executes convolution on a Grayscale8 image according to param: kernel.
    void convolution(uchar *in, float* kernel, int kWidth, int kHeight, uchar *out);
    // Overloaded convolution: normalizes values by lambda function f(x).
    void convolution(uchar* in, float* kernel, int kWidth, int kHeight, std::function<void(int*)> f, uchar* out);
    // Overloaded convolution: outputs (int) values. Useful when the tone range is wide and normalization is not straightforward.
    void convolution(uchar* in, float* kernel, int kWidth, int kHeight, int *out, int &min, int &max);

    // Binarizes a Grayscale8 image.
    void binarize(uchar* in, uchar* out);

    // Applies dynamic range compression to a Grayscale8 image. Output is to be normalized.
    void dynamic_range_compression(uchar* in, float c, float gamma, int* out);

    // Normalizes a Grayscale8 image.
    void normalize(int* in, int max, int min, uchar* out);
